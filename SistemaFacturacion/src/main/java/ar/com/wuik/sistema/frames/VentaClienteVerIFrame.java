...
public class VentaClienteVerIFrame extends WAbstractModelIFrame {
	...
	private static final Logger LOGGER = LoggerFactory
			.getLogger(VentaClienteVerIFrame.class);
	...
	public VentaClienteVerIFrame(VentaClienteIFrame facturaIFrame,
		...
		getTxtFechaEmision().setText(WUtils.getStringFromDate(new Date()));
		...
		// String nroFactura = parametroBO.getNroFactura();
		// getTxtNro().setText(nroFactura);
		// } catch (BusinessException bexc) {
		// LOGGER.error("Error al obtener Nro. de Factura", bexc);
		// }
	}
				...
				public void actionPerformed(ActionEvent e) {
					...
					if (validateModel(model)) {
						...
						try {
							...
							LOGGER.error("Error al guardar la Factura", bexc);
							WFrameUtils
									.showGlobalErrorMsg("Se ha producido un error al guardar la Factura");
						}
					}
				}
			});
