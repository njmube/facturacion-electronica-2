...
public class NotaDebitoVistaIFrame extends WAbstractModelIFrame {
	/**
	 * Serial UID.
	 */
	 ...
	 */
	public NotaDebitoVistaIFrame(Long idFactura) {
		initialize("Ver Nota de Débito");
		WModel model = populateModel();
		NotaDebitoBO notaDebitoBO = AbstractFactory.getInstance(NotaDebitoBO.class);
		try {
			this.notaDebito = notaDebitoBO.obtener(idFactura);
			model.addValue(CAMPO_FECHA_EMISION,
					WUtils.getStringFromDate(notaDebito.getFechaVenta()));
			model.addValue(CAMPO_OBSERVACIONES, notaDebito.getObservaciones());
			model.addValue(CAMPO_CAE, notaDebito.getCae());
			model.addValue(CAMPO_CAE_FECHA,
					WUtils.getStringFromDate(notaDebito.getFechaCAE()));
			if (notaDebito.isActivo()) {
				model.addValue(
						CAMPO_ESTADO,
						"ACTIVA"
								+ (WUtils.isEmpty(notaDebito.getCae()) ? " - SIN FACTURAR"
										: " - FACTURADA"));
			} else {
				model.addValue(
						CAMPO_ESTADO,
	...
	protected void refreshFacturas() {
		getTblRemitos().addData(new ArrayList<Factura>(notaDebito.getFacturas()));
	}
	...
	private void initialize(String title) {
		...
		setFrameIcon(new ImageIcon(
				NotaDebitoVistaIFrame.class.getResource("/icons/notas_credito.png")));
	...
	private JButton getBtnCerrar() {
		if (btnCerrar == null) {
			...
			btnCerrar.setIcon(new ImageIcon(NotaDebitoVistaIFrame.class
	...
	private WTablePanel<DetalleNotaDebito> getTblDetalle() {
		if (tblDetalle == null) {
			tblDetalle = new WTablePanel(DetalleNotaDebitoModel.class, "Detalles");
	...
	private void calcularTotales() {
		...
		getTxtSubtotalPesos().setText(
				WUtils.getValue(notaDebito.getSubtotal()).toEngineeringString());
		getTxtIVA10().setText(
	...
	private WTablePanel<Factura> getTblRemitos() {
		if (tblRemitos == null) {
			tblRemitos = new WTablePanel(DetalleNotaDebitoFacturaModel.class,
					"Facturas");
