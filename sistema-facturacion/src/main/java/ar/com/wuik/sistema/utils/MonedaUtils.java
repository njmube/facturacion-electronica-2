...
public class MonedaUtils {
	 ...
	 *
	 * @param numero
	 *            El número a convertir a letras.
	...
	public static String enLetras(BigDecimal numero) {
		...
		BigDecimal decpart = numero.subtract(new BigDecimal(numero
				.toBigInteger()));
	...
	private final static String[] UNIDADES = { "cero", "uno", "dos", "tres",
			"cuatro", "cinco", "seis", "siete", "ocho", "nueve" };
	private final static String[] DIECIS = { "diez", "once", "doce", "trece",
			"catorce", "quince", "dieciséis", "diecisiete", "dieciocho",
			"diecinueve" };
	private final static String[] VEINTIS = { "veinte", "veintiuno",
			"veintidós", "veintitrés", "veinticuatro", "veinticinco",
			"veintiséis", "veintisiete", "veintiocho", "veintinueve" };
	private final static String[] DECENAS = { "ERROR", "diez", "veinte",
			"treinta", "cuarenta", "cincuenta", "sesenta", "setenta",
			"ochenta", "noventa" };
	private final static String[] CENTENAS = { "ERROR", "ciento", "doscientos",
			"trescientos", "cuatrocientos", "quinientos", "seiscientos",
			"setecientos", "ochocientos", "novecientos" };
	private final static String[] PERIODOS = { "ERROR", "mi", "bi", "tri",
			"quatri", "quinti", "sexti", "septi", "octi", "noni", "deci",
			"undeci", "duodeci", "tredeci", "cuatrodeci", "quindeci",
			"hexadeci", "septendeci", "octodeci", "novendeci", "viginti",
			"unviginti", "duoviginti", "treviginti", "cuatroviginti",
			"quinviginti", "sexviginti", "septenviginti", "octoviginti",
			"novemviginti", "triginti", "untriginti", "duotriginti",
			"tretriginti" };
	...
	private static List<PeriodoNumerico> splitPeriodos(String digitos) {
		...
		List<PeriodoNumerico> lista = new ArrayList<PeriodoNumerico>();
	...
	/*
	 * Capitaliza la primera palabra y pone exactamente un espacio entre
	 * palabras
	 */
	...
	private static class PeriodoNumerico {
		...
		public PeriodoNumerico(int posicion, Integer numero) {
			...
			if (posicion < 0) {
				throw new IllegalArgumentException(
						"Posicion debe ser un numero positivo!");
			}

			if (numero < 0) {
				throw new IllegalArgumentException(
						"Periodo numérico no puede ser menor que cero!");
			}
			if (numero > 999999) {
				throw new IllegalArgumentException(
						"Periodo numérico no puede ser mayor que 999.999!");
			}
		...
		private String sufijo() {
			...
			default:
				...
				return sb2.append(PERIODOS[m_posicion]).append(LLONES)
						.toString();
			}
		}
		...
		private List<ClaseNumerica> splitClases(Integer s) {
			...
			List<ClaseNumerica> lc = new ArrayList<ClaseNumerica>();
