 ...
 */
public abstract class WAbstractModelFrame extends WAbstractFrame {


	/**
	 * Serial UID.
	 */
	private static final long serialVersionUID = 1461372623409583236L;
	private static final Logger LOGGER = LoggerFactory.getLogger( WAbstractModelFrame.class );
						...
						if ( WUtils.isNotEmpty( comps ) ) {
							for ( Component subcomp: comps ) {
								if ( subcomp instanceof JViewport ) {
									Component[] subsubcomps = ( (JViewport) subcomp ).getComponents();
			...
			for ( Component component: components ) {
				if ( null != component.getName() ) {
					...
					if ( component instanceof JTextComponent ) {
						...
						JComboBox jC = (JComboBox) component;
						if ( null != value ) {
							jC.setSelectedItem( new WOption( Long.valueOf( value.toString() ) ) );
						}
					} else if ( component instanceof JCheckBox ) {
						JCheckBox jC = (JCheckBox) component;
						if ( null != value ) {
										...
										if ( subsubcomp instanceof JTextArea ) {
											...
											if ( null != value ) {
												...
												for ( int i = 0; i < size; i++ ) {
													WOption item = (WOption) list.getModel().getElementAt( i );
													if ( items.contains( item.getValue() ) ) {
														selectedIndexes.add( i );
													}
												}
												list.setSelectedIndices( WUtils.toPrimitive( selectedIndexes
